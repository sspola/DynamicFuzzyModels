...
class Model_Simulator(object):
	...
	def fitness(self, x, SIM=None):
		...
		return end_apo-begin_apo, end_nec-begin_nec, complexity
...
if __name__ == '__main__':
	...
	final_results = [(s.objectives[0], s.objectives[1], s.objectives[2]) for s in algorithm.result]
	final_solutions = []
	...
	ax.scatter3D([s.objectives[0] for s in array(algorithm.result)[pareto_front]],
	            [s.objectives[1] for s in array(algorithm.result)[pareto_front]],
				[s.objectives[2] for s in array(algorithm.result)[pareto_front]],
	            label="Pareto front", color="red")

	ax.set_xlabel("Change in Apoptosis")
	ax.set_ylabel("Change in Necrosis")
	ax.set_zlabel("Complexity of perturbation")
	ax.legend()
	plt.show()